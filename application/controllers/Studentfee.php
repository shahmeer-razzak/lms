...
class Studentfee extends Admin_Controller
{
    ...
    public function addFeeDiscount()
    {
        ...
        $this->form_validation->set_rules('discount_amount', 'discount_amount', 'required|trim|xss_clean');


        if ($this->form_validation->run() == false) {
            $data = array(
                'fee_groups_feetype_id'  => form_error('fee_groups_feetype_id'),
                'discount_amount' => form_error('discount_amount'),
            ...
            $result =  $this->Feegrouptype_model->addDiscountFee($fee_groups_feetype_id, $discount_amount);
            if ($result) {
    ...
    public function createfinalDueDateById()
    {
        if (!$this->rbac->hasPrivilege('fees_discount_assign', 'can_view')) {
            access_denied();
        }
        $this->form_validation->set_rules('fee_groups_feetype_id', $this->lang->line('fee_groups_feetype_id'), 'required|trim|xss_clean');
        $this->form_validation->set_rules('final_due_date', 'please add the final due date', 'required|trim|xss_clean');

        if ($this->form_validation->run() == false) {
            $data = array(
                'fee_groups_feetype_id'  => form_error('fee_groups_feetype_id'),
                'final_due_date' => form_error('final_due_date'),
            );
            $array = array('status' => 'fail', 'error' => $data);
            echo json_encode($array);
        } else {

            $final_due_date           = $this->input->post('final_due_date');
            $this->Feegrouptype_model->addfinalDueDateById($final_due_date, $fee_groups_feetype_id);
