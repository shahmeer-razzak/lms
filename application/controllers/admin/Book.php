...
class Book extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('accession_no', $this->lang->line('accession_no'), 'trim|required|xss_clean|is_unique[books.accession_no]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'corporate_author'      => $this->input->post('corporate_author'),
    ...
    {
        $data['fields'] = array('book_title',  'classification_no', 'isbn_no', 'subject', 'rack','place', 'accession_no','publication_year','volume', 'pages', 'item_type','book_language', 'vendor',  'publish', 'edition', 'author', 'author_mark', 'sub_title', 'corporate_author',  'qty', 'perunitcost', 'postdate',  'available');
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        foreach ($result as $r_key => $r_value) {
                            ...
                            $result[$r_key]['classification_no']     = $this->encoding_lib->toUTF8($result[$r_key]['classification_no']);
                            ...
                            $result[$r_key]['rack']     = $this->encoding_lib->toUTF8($result[$r_key]['rack']);
                            $result[$r_key]['place']     = $this->encoding_lib->toUTF8($result[$r_key]['place']);
                            
                            $result[$r_key]['accession_no']     = $this->encoding_lib->toUTF8($result[$r_key]['accession_no']);
                            
                            $result[$r_key]['publication_year']     = $this->encoding_lib->toUTF8($result[$r_key]['publication_year']);
                            
                            $result[$r_key]['volume']     = $this->encoding_lib->toUTF8($result[$r_key]['volume']);
                            $result[$r_key]['pages']     = $this->encoding_lib->toUTF8($result[$r_key]['pages']);
                            
                            $result[$r_key]['item_type']     = $this->encoding_lib->toUTF8($result[$r_key]['item_type']);
                            $result[$r_key]['book_language']     = $this->encoding_lib->toUTF8($result[$r_key]['book_language']);
                            
                            $result[$r_key]['vendor']     = $this->encoding_lib->toUTF8($result[$r_key]['vendor']);
                            ...
                            $result[$r_key]['author_mark']      = $this->encoding_lib->toUTF8($result[$r_key]['author_mark']);

                            $result[$r_key]['sub_title']      = $this->encoding_lib->toUTF8($result[$r_key]['sub_title']);


                            $result[$r_key]['corporate_author']      = $this->encoding_lib->toUTF8($result[$r_key]['corporate_author']);
            ...
            foreach ($m->data as $key => $value) {
              ...
              //  if ($value->description == "") {
               ...
               // $row[]     = $value->book_no;
                ...
                $row[]     = $value->author_mark;
                $row[]     = $value->sub_title;
                
                $row[]     = $value->corporate_author;
                $row[]     = $value->subject;
                            $row[]     = $value->rack;
                $row[]     = $value->place;
                
                $row[]     = $value->accession_no;
                $row[]     = $value->publication_year;
                $row[]     = $value->volume;
                $row[]     = $value->pages;
                $row[]     = $value->item_type;
                $row[]     = $value->book_language;
                $row[]     = $value->vendor;
