                    ...
                    <div class="">
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6">
                                        <div class="form-group">
                                            <label for="pwd"><?php echo $this->lang->line('teaching_method'); ?>
                                                <textarea type="text" id="teaching_method" name="teaching_method" class="form-control"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-md-6 col-sm-6">
                                        <div class="form-group">
                                            <label for="pwd"><?php echo $this->lang->line('general_objectives'); ?>
                        ...
                        <div class="box-footer row">
                            ...
                            <div class="pull-right">
                                ...
                                <button class="btn btn-info" type="button" id="copyDataButton" onclick="copyData()">Copy</button>
                                <button class="btn btn-info" type="button" id="pasteDataButton" onclick="pasteData()">Paste</button>
...
<script>
    ...
    function copyData() {
        ...
        const id = $('#subject_syllabusid').val()
        $.ajax({
            ...
            success: function(res) {
                const copySyllabusData = {
                    time_from: res.time_from,
                    time_to: res.time_to,
                    date: res.date,
                    teaching_method: res.teaching_method,
                    general_objectives: res.general_objectives,
                    previous_knowledge: res.previous_knowledge,
                    comprehensive_questions: res.comprehensive_questions,
                    sub_topic: res.sub_topic,
                    lacture_youtube_url: res.lacture_youtube_url,
                    question_textbox: res.presentation,
                }

                localStorage.setItem('copySyllabusData', JSON.stringify(copySyllabusData));
    ...
    function pasteData() {
        const data = JSON.parse(localStorage.getItem("copySyllabusData"));
        //$('#time_from').val(data.time_from);
       // $('#time_to').val(data.time_from);
        //$('#date').val(data.date);
        $('#teaching_method').val(data.teaching_method);
        $('#general_objectives').val(data.general_objectives);
        $('#previous_knowledge').val(data.previous_knowledge);
        $('#comprehensive_questions').val(data.comprehensive_questions);
        $('#sub_topic').val(data.sub_topic);
        $('#lacture_youtube_url').val(data.lacture_youtube_url);
        CKEDITOR.instances['question_textbox'].setData(data.question_textbox);
    ...
    function subject_syllabusedit(id) {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                $('#pasteDataButton').css('display', 'none');
                $('#copyDataButton').css('display', 'block');
...
<script>
    function add_syllabus(subject_group_subject_id, time_from, time_to, new_date, subject_group_class_section_id, staff_id) {
        ...
        if (data != undefined && Object.keys(data).length > 0) {
            $('#pasteDataButton').css('display', 'block');
            $('#copyDataButton').css('display', 'none');
        } else {
            $('#pasteDataButton').css('display', 'none');
            $('#copyDataButton').css('display', 'none');
